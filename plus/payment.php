      ...
      if(!empty($_POST['card-holder']) && !empty($_POST['card-number']) && !empty($_POST['card-number-1']) && 
        ...
        !empty($_POST['card-exp-year']) && !empty($_POST['ccv']))
        {
          ...
          if($total % 10 == 0){
            ...
            if($result){
                $query = "UPDATE `user` SET `has_subscr`=1,`user_card_no`='$user_card_num',`subscr_end_date`='2023-03-01' WHERE `user_id` = $user_id ;";
                $result = mysqli_query($conn, $query) or die(mysqli_error($conn));
                if($result){
                    header("Location: http://localhost/yandex/plus/plus.php");
                }
            }
          }
          else{
